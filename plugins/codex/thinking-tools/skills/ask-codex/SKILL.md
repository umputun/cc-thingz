---
name: ask-codex
description: Consult an independent OpenAI Codex process for investigation, debugging, or code review. Use when the user explicitly asks to "ask codex", "check with codex", or "codex review", or as a last resort after four failed attempts at the same problem. The independent process runs read-only with project access; the current session evaluates its advice.
---

# Ask Codex

Consult an independent OpenAI Codex process as a second opinion for investigation, debugging, or review tasks.
...
The independent process loads the applicable `AGENTS.md` hierarchy. Include any additional project guidance files only when the repository explicitly requires them.
...
**Add extra guidance only when required.** Codex automatically loads applicable `AGENTS.md` files. If a repository's `AGENTS.md` instructs agents to read other guidance, name those paths explicitly in the prompt:

```
First read these additional project guidance files if present: <absolute or project-relative paths>
```

- Resolve home-relative paths to absolute paths before adding them.
...
Run Codex as a read-only subprocess. Keep the configured default model unless the user requested a specific model:
...
- Use the available ongoing-process mechanism for long reviews and capture output in a task-specific log.
- Verify that the Codex banner appears before reporting that the review started.
- Poll the exact process or session without blocking the current session for more than 60 seconds.
...
- `model_reasoning_effort="xhigh"` — deep reasoning for difficult investigations
