...
**Important**: Avoid `$()` command substitution when running shell commands. Use sequential steps.
...
After displaying the table, ask the user to choose one numbered option:

```
1. All commits: show full details for each commit.
2. None: skip details.
3. Specific commit: ask for the commit hash to inspect.
```
...
**If "Specific commit"**: Run the same `git show` command for that commit only.
