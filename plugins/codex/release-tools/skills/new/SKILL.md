...
Ask the user to choose one numbered release type, and wait for the answer:

```text
1. Hotfix: bug fixes (1.2.3 → 1.2.4).
...
```bash
platform=$(bash <plugin-root>/skills/new/scripts/detect-platform.sh)
```
...
```bash
new_version=$(bash <plugin-root>/skills/new/scripts/calc-version.sh <release_type>)
```
...
```bash
notes=$(bash <plugin-root>/skills/new/scripts/get-notes.sh "$platform")
```
...
**Post-processing (the agent must do this before presenting):**
...
Ask the user to choose one numbered option, and wait for the answer:

```text
1. Yes, publish: create the tag and publish the release.
2. Cancel: abort the release.
```
