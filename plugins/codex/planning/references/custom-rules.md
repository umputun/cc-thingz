...
2. **User-level**: `${CODEX_HOME:-$HOME/.codex}/cc-thingz/planning/planning-rules.md`
...
Each skill runs `resolve-rules.sh planning-rules.md` via Bash at startup. The script uses `$CODEX_HOME` when set and otherwise defaults to `$HOME/.codex`. It outputs the first file found (project, then user) or empty output if neither exists.
...
- **add/update user rules** — writes to `${CODEX_HOME:-$HOME/.codex}/cc-thingz/planning/planning-rules.md`
- **clear project rules** — deletes `.codex/planning-rules.md`
- **clear user rules** — deletes `${CODEX_HOME:-$HOME/.codex}/cc-thingz/planning/planning-rules.md`
