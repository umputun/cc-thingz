...
interactive plan review hook that lets you annotate the agent's plans directly
in your editor before approving them. when the agent calls ExitPlanMode, this
...
the hook computes a unified diff and sends it back to the agent as a denial
reason, forcing the agent to revise the plan based on your annotations. if
you make no changes, the normal approval dialog appears.

this creates a feedback loop: annotate → the agent revises → annotate again →
...
any text change works - the hook diffs original vs edited and the agent sees
...
limitations:
  ...
  - plan content comes from the agent's ExitPlanMode call, not from the plan
...
the unified diff to stdout (no JSON wrapping). the agent reads the diff,
...
def open_editor(filepath: Path, target_window: bool = True) -> int:
    ...
    # requires KITTY_LISTEN_ON for socket communication — Codex runs
