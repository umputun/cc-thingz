...
Use this for the finalize agent after all reviews pass (replace `DEFAULT_BRANCH`, `PLAN_FILE_PATH`, `PROGRESS_FILE_PATH`, and `<plugin-root>`):

```
Post-completion finalize step. Organize commits for merge.

AUTONOMOUS MODE — NO HUMAN IS AVAILABLE: run unattended and never ask the user anything or pause for input or approval. On a rebase conflict or a squash judgment call, resolve it yourself; if it is not safe to resolve, abort cleanly (`git rebase --abort`) and report instead of waiting for input.
...
STEP 4 - LOG PROGRESS:
Log results: bash <plugin-root>/skills/exec/scripts/append-progress.sh PROGRESS_FILE_PATH "finalize: completed"
Then pipe details: echo "- rebase: <success/failed>
- commits before: N, after: M
- squashed: <list of squashed commits, or none>
- validation: <passed/failed>" | bash <plugin-root>/skills/exec/scripts/append-progress.sh PROGRESS_FILE_PATH
