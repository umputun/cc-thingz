...
Use the available collaboration mechanism to launch one analysis subagent. Pass all context the subagent needs in the prompt:
...
Activate the `review:writing-style` skill for proper tone.
...
→ Phase 1.5: ask "Review mode?" → user selects Quick
...
→ Phase 3: present condensed report
→ Phase 4: ask "Section type change: Accept typed approach?"
  → User: "Accept"
→ User: "draft the review"
