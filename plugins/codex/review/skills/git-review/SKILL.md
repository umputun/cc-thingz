...
`.codex-plugin/plugin.json`. Resolve it to an absolute path before running the bundled script.
...
5. The agent reads annotations and fixes code in the real repository
...
```bash
<plugin-root>/skills/git-review/scripts/git-review.py [base_ref]
```
...
Prepare a concise implementation plan for the annotations:
...
→ propose: "annotation requests input validation in handler.go; add a validate() call"
→ user approves plan
→ the agent adds input validation to the handler
