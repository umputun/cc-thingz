...
ordinary output outside the choice prompt. The prompt covers roughly five lines above
...
3. Take the items ONE AT A TIME: print that item's briefing, ask about it alone
...
3. Report every item, one line each — `yes` first, then `later`, then `no`, oldest `added` first within
   ...
   its reasoning.
4. Offer concrete actions as explicit choices, with a recommendation first:
...
step as explicit choices, staging the exact paths this invocation created or edited and not the
