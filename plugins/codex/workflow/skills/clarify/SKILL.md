...
- Resolution: suggest updating docs and prepare a plan when the change is not trivial
...
When there are multiple valid solutions, present explicit choices:
...
1. Create an implementation plan for the chosen approach, using `planning:make` when available
2. Plan should reflect the scope assessment from Step 1

**CRITICAL**: Do not attempt to fix issues without planning. Always prepare a plan for:
- Bug fixes
- Design changes
- Missing features
...
[If real issue]:
This is a real issue that should be addressed. I recommend preparing an implementation plan.

Should I prepare the plan?
```
...
- **Prepare an implementation plan when investigation confirms a genuine bug or flaw**
