...
Always substitute: `PLAN_FILE_PATH`, `PROGRESS_FILE_PATH`, `DEFAULT_BRANCH`, `${CLAUDE_PLUGIN_ROOT}` (resolve to actual absolute path), and phase-specific values (`CODEX_OUTPUT`, `PREVIOUS_CONTEXT`, `FINDINGS_LIST`).
...
3. **If none found** — all tasks complete, go to step 4
...
This is an adversarial loop: codex (GPT-5) reviews the code, Claude evaluates findings and fixes real issues, codex re-reviews. Loop until codex finds nothing.

See `prompts/codex-review.md` for the full flow.
...
1. **Run codex** — execute the codex command from `prompts/codex-review.md` via Bash tool with `run_in_background: true`. You will be automatically notified when it completes — do NOT poll, sleep, or check status. Pass the diff instruction (iteration 1: `git diff <base>...HEAD`, subsequent: `git diff`). Capture codex output.

2. **Check codex output** — if codex reports "NO ISSUES FOUND" or equivalent, phase is done.

3. **Report codex findings to user** — show a compact list (one line per finding).

4. **Spawn evaluator agent** — launch one Agent tool call with `mode: "bypassPermissions"`, `subagent_type: "general-purpose"`, and the evaluator prompt from `prompts/codex-review.md`. Pass the full codex output. The evaluator:
   - Verifies each finding by reading actual code
   - Fixes confirmed issues
   - Explains away false positives (explanations become context for next codex run)
   - Runs validation commands from the plan file
   - Commits fixes if any
   - Logs details to progress file

5. **After evaluator returns** — report to user what was fixed (compact list from evaluator's response). Pass evaluator's response as `PREVIOUS_CONTEXT` for the next codex iteration.

6. Log to progress file and loop back to step 1.
